...
def fetch_biotech_news(lookback_hours: int = 24) -> list:
    """
    여러 바이오테크 뉴스 소스(Nature, FierceBiotech 등)에서 최신 기술 뉴스를 가져옵니다.
    ...
    for source in rss_sources:
        logger.info(f"{source['name']} RSS 피드 가져오는 중: {source['url']}")
        try:
    ...
    logger.info(f"총 {len(all_news_items)}개의 최신 바이오테크 뉴스를 찾았습니다.")
