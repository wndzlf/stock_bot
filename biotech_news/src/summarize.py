...
def summarize_biotech_news(news_items: list) -> str:
    ...
    - 중요한 메커니즘/수치/핵심 단어는 이탤릭 효과로 강조 (Unicode Mathematical Italic 사용: 𝑖𝑡𝑎𝑙𝑖𝑐 𝑡𝑒𝑥𝑡 이런 식으로 써. 예: 𝟵𝟵% 정확도 달성)
    - **중요: 모든 마크다운 기호(별표 *, 언더바 _, 물결 ~~ 등)를 절대 쓰지 마.** X(트위터)용이므로 기호 없이 오직 Unicode 변환 문자만 사용해야 함.
