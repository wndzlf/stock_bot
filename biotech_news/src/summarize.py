...
def summarize_biotech_news(news_items: list) -> str:
    ...
    필수 룰 – 절대 어기지 마:
    - 첫 문장은 볼드 효과를 주어 강하게 시작 (Unicode Sans-serif Bold 사용: 𝗕𝗢𝗟𝗗 𝗧𝗘𝗫𝗧 이런 식으로 써. 예: 𝗔𝗜를 활용한 난치병 치료제 설계 성공)
    - 중요한 메커니즘/수치/핵심 단어는 이탤릭 효과로 강조 (Unicode Sans-serif Italic 사용: 𝘪𝘵𝘢𝘭𝘪𝘤 𝘵𝘦𝘹𝘵 이런 식으로 써. 예: 𝘗𝘢𝘳𝘢𝘥𝘪𝘨𝘮 𝘚𝘩𝘪𝘧𝘵 혹은 𝟫𝟵% 정확도)
    - **중요: 모든 서체 스타일링(볼드, 이탤릭)은 영문(Latin)과 숫자만 지원하므로, 한글에 스타일링을 시도해서 글자가 깨지거나(𝗕면역 등) 스타일이 섞이지 않게 주의해.** 영문 단어 전체에 적용하거나 생략할 것.
    - **중요: 모든 마크다운 기호(별표 *, 언더바 _, 물결 ~~ 등)를 절대 쓰지 마.** X(트위터)용이므로 오직 Unicode 변환 문자만 사용해야 함.
    ...
    - 말투는 전문적이고 단호하게 (Slang, ㅋㅋㅋ, ㄹㅇ 등 가벼운 표현 절대 금지).
