...
def summarize_biotech_news(news_items: list) -> str:
    ...
    필수 룰 – 절대 어기지 마:
    - 첫 문장은 볼드 효과를 주어 강하게 시작 (Unicode Mathematical Bold 사용: 𝗕𝗢𝗟𝗗 𝗧𝗘𝗫𝗧 이런 식으로 써. 예: 𝗔𝗜를 활용한 난치병 치료제 설계 성공)
    - 중요한 메커니즘/수치/핵심 단어는 이탤릭 효과로 강조 (Unicode Mathematical Italic: *𝑖𝑡𝑎𝑙𝑖𝑐 𝑡𝑒𝑥𝑡* 이런 식. 예: 𝟵𝟵% 정확도 달성)
    - **중요: Markdown 기호(**, *, _, ~~ 등)를 절대 사용하지 마.** X(트위터)는 마크다운을 지원하지 않으므로, 기호 없이 Unicode 볼드/이탤릭 문자만 사용해야 함.
